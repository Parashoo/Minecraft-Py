...
class chunk:
    ...
    def __init__(self, *args, gen=False):
        if gen:
            self.data = np.zeros((18, 64, 18), dtype="uint8")
            ...
            self.render_array = np.zeros((16, 64, 16, 6), dtype = "int16")
    ...
    def return_exposed(self):
        ...
        for i in range(64):
            if not np.all(self.data[:16,63-i,:16] == np.zeros((16, 16), dtype = 'uint8')):
                self.top_block_layer = 64-i
                break
            else: continue
    ...
    def return_exposed_t(self, ctx, transform_program):
        ...
        self.exposed_t = ctx.buffer(reserve = 16 * 64 * 16 * 6 * 4) # 32-bit float
        self.data_tex_data = np.concatenate((np.zeros((18, 1, 18)), self.data, np.zeros((18, 1, 18))), axis=1, dtype='f4')
        print(self.data[:,1,:])
        data_tex = ctx.texture3d((18, 66, 18), 1, data=self.data_tex_data, dtype = 'f4')
